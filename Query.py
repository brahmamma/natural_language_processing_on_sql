...
class Select():
	...
	def print_column(self, selection):
		...
		if column is None:
			...
			if column_type == 'COUNT':
				return   'COUNT('  + str(column) + color.BOLD + ')'
			elif column_type == 'AVG':
				return  'AVG('+ str(column) + color.BOLD + ')'
			...
			if len(self.columns) == 1:
				...
				for i in range(0, len(self.columns)):
					if i == (len(self.columns)-1):
						...
						#output1.write(str(self.columns[i][1])+" ")
						output.write('\t\t\t}\n')
					else:
	...
	def print_json(self, output):
		if len(self.tables) >= 1:
			if len(self.tables) == 1:
				...
				#output1.write(str(self.tables[0]+" ")
				output.write('\t},\n')
			else:
				output.write('\t"join": {\n')
				#output1.write("join"+" ")
				...
				for i in range(0, len(self.tables)):
					if i == (len(self.tables)-1):
						#output1.write(str(self.tables[i]))
...
class Condition():
	...
	def print_json(self, output):
		...
		#output1.write( self.get_just_column_name(str(self.column))+" ")
		#output1.write(str(self.column_type)) if ((str(self.column_type))!="None") else output1.write(" ")
		#output1.write(str(self.operator)+" ")
		#output1.write(str(self.value)+" ")
			...
			if len(self.conditions) == 1:
				...
				for i in range(0, len(self.conditions)):
					if i != 0:
						...
						#output1.write(str(self.conditions[i][0]) +" ")
	...
	def print_json(self, output):
		if len(self.columns) >= 1:
			if len(self.columns) == 1:
				...
				#output1.write("order by"+" ")
				#output1.write(str(self.columns[0][1]))
				output.write('\t},\n')
			else:
				output.write('\t"select": {\n')
				output.write('\t\t"columns": [\n')
				for i in range(0, len(self.columns)):
					if i == (len(self.columns)-1):
						...
						#output1.write("order by"+" "+str(self.columns[i][1]))
						output.write('\t\t\t}\n')
					else:
						output.write('\t\t\t{ "column": "' + str(self.columns[i][0]) + '",\n')
						output.write('\t\t\t  "order": "' + str(self.columns[i][1]) + '"\n')
...
class Query():
	...
	def print_json(self, filename="output.json",txtfile="output.txt"):
		output = open(filename, 'a')
		output1=open(txtfile,'a')
