...
class ln2sql:
    def __init__(self, database_path, input_sentence, language_path, thesaurus_path, json_output_path,txt_output_path):
        database = Database()
        ...
        if json_output_path is not None:
            ...
            self.remove_txt(txt_output_path)
            for query in queries:
                query.print_json(json_output_path,txt_output_path)
    ...
    def remove_json(self, filename="output.json"):
        if os.path.exists(filename):
            os.remove(filename)
    def remove_txt(self,filename="output.txt"):
        if os.path.exists(filename):
            os.remove(filename)
